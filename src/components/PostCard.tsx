...
export function PostCard({ post, index, state, disabled, onChange }: Props) {
  ...
  return <article id={post.id} data-post-id={post.id} aria-labelledby={`title-${post.id}`} className="post-card" tabIndex={-1}>
    <div className="post-body">
      <div className="post-meta"><span className={`topic topic-${index % 4}`} title={place ? `${place.umbrella.label} › ${place.field.label}` : post.topic}>{place && place.field.id !== "general" ? place.field.label : post.topic}{post.subtopic && place?.field.id !== "general" ? <span className="subtopic"> · {post.subtopic}</span> : null}</span><span className="sample">{post.status === "published" ? post.contentType === "news" ? "NEWS" : "EVERGREEN" : "SAMPLE"}</span><span className="post-number">{String(index + 1).padStart(2, "0")}</span></div>
